...
export class DeFiModule {
  ...
  async getProtocolsData(): Promise<Record<string, any>[]> {
    ...
    if (!cachedProtocolsData || cachedProtocolsData.timestamp + deadline > Date.now()) {
      const protocols = await client.get("/protocols").then(resp => resp.data) as Record<string, any>[];
      const filteredProtocols = protocols.map(protocol => filterKeys(protocol, [
        "name",
        "description",
        "url",
        "logo",
  ...
  async getChainsTvl(): Promise<Record<string, any>[]> {
    const chains = await client.get("/v2/chains").then(resp => resp.data) as Record<string, any>[];
    const filteredChains = chains.sort((a, b) => b.tvl - a.tvl).map(chain => filterKeys(chain, [
      "tvl",
      "name",
      "chainId",
    ]));
  ...
  async getDefaultProtocols() {
    return (await this.getProtocolsData()).filter(obj => this.defaultProtocols.includes(obj.slug));
  }
}
